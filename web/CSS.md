...
- float으로 처리한 엘리먼트는 블록으로 인식되지 않기 때문에 그 아래에 있는 엘리먼트가 위로 올라오는 문제가 발생한다. 이때 clear 속성을 부여하면 해당 태그의 float를 인식할 수 있게 된다.
- 자식이 float인 태그는 자식으로 인정하지 않기 때문에 높이값을 부여하지 않는데, overflow라는 속성을 부여하면 해당 태그의 float을 부모 입장에서 인식할 수 있게 된다. 이때는 부모 태그에 overflow:auto 라고 부여하면 된다.
- float를 사용하면 세 개의 블록을 차례로 배치할 수 있게 된다.
